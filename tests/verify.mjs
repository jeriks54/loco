...
assert.equal(new Set(levels.map(l => l.id)).size, 19);
assert.deepEqual(levels.map(l => l.id), [...Array.from({ length: 7 }, (_, i) => `ch1-0${i + 1}`), ...Array.from({ length: 8 }, (_, i) => `ch2-0${i + 1}`), ...Array.from({ length: 4 }, (_, i) => `ch3-0${i + 1}`)]);
...
console.log(`PASS: ${sourceFiles.length} module syntax checks; 19 level paths; 8 counted + 4 sensor solution checks; executor regression checks.`);
